                ...
                <div className="flex flex-col md:flex-row">
                    ...
                    <div className="hidden md:flex flex-col space-y-4 mr-6">
                        {selectedProducts.images.map((image, index) => (
                            <img onClick={() => setMainImage(image.url)} className={`w-20 h-20 object-cover rounded-lg cursor-pointer border ${mainImage === image.url ? "border-black" : "border-gray-300"}`} key={index} src={image.url} alt={image.altText || `Thumbnail ${index}`} />
                        ))}
                    </div>
                    {/* Main Image */}
                    ...
                    {/* Mobile Thumbnails */}
                    <div className="md:hidden flex overscroll-x space-x-4 mb-4">
                        {selectedProducts.images.map((image, index) => (
                            <img onClick={() => setMainImage(image.url)} className={`w-20 h-20 object-cover rounded-lg cursor-pointer border ${mainImage === image.url ? "border-black" : "border-gray-300"}`} key={index} src={image.url} alt={image.altText || `Thumbnail ${index}`} />
                        ))}
                    </div>
